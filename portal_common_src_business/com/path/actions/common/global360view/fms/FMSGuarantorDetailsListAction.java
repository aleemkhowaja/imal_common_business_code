...
public class FMSGuarantorDetailsListAction extends GridBaseAction
{
  ...
  public String showFMSGuarantorGrid()
  {
      try
      {
    	  ...
    	  fmsCOList = dashFMSBO.getCifGuarantorCurrencyDetails(dashboardSC);
    	  
	   }
      catch(Exception ex)
      {
    	  log.error("Error in load showFMSGuarantorGrid Page");
	  handleException(ex, null, null);
      }
      return SUCCESS;
  }

  public String loadFMSGuarantorGrid()
	{
	      String[] searchCol = {};
	      try
		{
		    ...
		    {
				setRecords(dashFMSBO.getCifGuarantorListCount(dashboardSC));
		    }
		    fmsCOList = dashFMSBO.getCifGuarantorList(dashboardSC);
		   ...
		   if(fmsCOList!=null && !fmsCOList.isEmpty())
		   {
			   for(FMSCO eachFacilityCO:fmsCOList)
			   {
				   ...
				   BigDecimal value 	 = NumberUtil.nullToZero(eachFacilityCO.getINDICATOR_AMOUNT());
		   ...
		   gridFooterMap.put("INDICATOR_AMOUNT", sumOfValueAmt);
		   setUserdata(gridFooterMap);
			
		}
		catch(Exception e)
		{
			log.error(e, "Error in FMS Guarantor List Grid");
