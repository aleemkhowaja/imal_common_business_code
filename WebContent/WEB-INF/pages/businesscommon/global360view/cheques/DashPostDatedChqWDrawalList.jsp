...
<%@include file="/WEB-INF/pages/common/TLDImport.jsp" %>
			...
			<ps:url id="urlPostDatedChqWDrawal_${_pageRef}" 
			        namespace="/path/globalview" escapeAmp="false"
				    action="dashPostDatedChqWDrawalListAction?_pageRef=${_pageRef}">
			   <ps:param name="cif_no"        value="criteria.cif_no"></ps:param>
			   <ps:param name="loginTypeByBr" value="criteria.loginTypeByBr"></ps:param>
			</ps:url>

		    <psjg:grid id="postDatedChqWDrawalGridTbl_Id_${_pageRef}" 
					caption="%{getText('postDatedChqWDrawal_key')}"
			        altRows="true"
			    	dataType="json" 
					filter="true"
			    	gridModel="gridModel" 
			    	href="%{urlPostDatedChqWDrawal_${_pageRef}}"
			        navigator="true" 
			        navigatorAdd="false"
				...
				<psjg:gridColumn id="trx_no"          	index="trx_no"            colType="number" name="trx_no" 	          				title="%{getText('Trx_No_key')}"	     	   editable="false" sortable="true" search="true" formatter="dashboard_postDatedChqWDrawalTrxBtn"/>
 ...
 var postDatedChqWdrawalKey = "<ps:text name='postDatedChqWDrawal_key'/>";
 $("#postDatedChqWDrawalGridTbl_Id_"+_pageRef).jqGrid('setGroupHeaders', {
