...
$(document).ready(function() {	
	$("#outStandingDealsSection_"+_pageRef+".collapsibleContainer").collapsiblePanel();
...
function financingDealByProductApprGridStatusColor(cellValue, options, rowObject){	
 ...
 <div id="outStandingDealsSection_${_pageRef}" class="collapsibleContainer" title="<ps:text name="outstanding_deals_key" />" >
    ...
    <ps:url id="urlOutstandingFinancingDealsGridByProduct" action="financingDealsListAction_loadFinancingDealsByProductData" namespace="/path/globalview" escapeAmp="false">
	 ...
	 <psjg:grid
	    	id				="outstandingFinancingDealsByProductGridTbl_Id_${_pageRef}" 
	  	    href            ="%{urlOutstandingFinancingDealsGridByProduct}"  	    
	        dataType        ="json"       
	    	pager           ="false"
	    	sortable        ="true"
			filter          ="false"
	        ...
	        shrinkToFit     ="false" 
			height="230"
			onCompleteTopics="onOutstandingFinancingDealsGridLoad"   
	        >	 
	       ...
	       <psjg:gridColumn id="remainingAmt"	      index="remainingAmt"        name="remainingAmt"       title="%{getText('Remaining_Amt_key')}"          colType="number" sortable="true" search="true"  formatter="currencyFmatter" formatCol="currDecimalPoints"  />
 ...
 <script>
 $(document).ready(function () {
	   $("#outstandingFinancingDealsByProductGridTbl_Id_"+_pageRef).unsubscribe("onOutstandingFinancingDealsGridLoad");
	   var isNavConstructed = false;
		$("#outstandingFinancingDealsByProductGridTbl_Id_"+_pageRef).subscribe("onOutstandingFinancingDealsGridLoad",function(response,html){
			 if(!isNavConstructed)
			 {
				 $("#outstandingFinancingDealsByProductGridTbl_Id_"+_pageRef).jqGrid ('navButtonAdd', '#outstandingFinancingDealsByProductGridTbl_Id_'+_pageRef+'_pager',
											      {  
						 						  caption:"<ps:text name='Print_Statement__key' />",
												  buttonicon: "ui-icon-print",
												  title: "<ps:text name='Print_Statement__key' />",
											      onClickButton: function() {										      
											      printGrid("outstandingFinancingDealsByProductGridTbl_Id_"+_pageRef,"<ps:text name='Approved_Deals_key' />",["action"]);
