...
$(document).ready(function() {	
	$("#pendingTreasuryForexDealsSection_"+_pageRef+".collapsibleContainer").collapsiblePanel();

});

function treasuryForexDealsPendingGridActionFormatter(cellValue, options, rowObject){
	var hyperLinks="";
...
<div id="pendingTreasuryForexDealsSection_${_pageRef}" class="collapsibleContainer" title="<ps:text name="Pending_Deals_key" />" >
    
    <ps:url id="urlPendingTreasuryForexDealsGrid" action="financingDealsListAction_loadTreasuryForexDeals" namespace="/path/globalview" escapeAmp="false">
	 ...
	 <psjg:grid
	    	id				="pendingTreasuryForexDealsGridTbl_Id_${_pageRef}" 
	  	    href            ="%{urlPendingTreasuryForexDealsGrid}"  	    
	        dataType        ="json"       
	    	pager           ="false"
	    	sortable        ="true"
			filter          ="false"
	        ...
	        shrinkToFit     ="false" 
			height="230"
			onCompleteTopics="onPendingTreasuryForexDealsGridLoad" >
	        ...
	        <psjg:gridColumn name="action"	title="%{getText('actions_key')}" index="action"  colType="custom" 	editable="false" 	sortable="false" 	search="false"	id="actionEdit" 	 formatter="treasuryForexDealsPendingGridActionFormatter"  width="100" />
	  </psjg:grid>
 </div>    
 ...
 $(document).ready(function () {
	   $("#pendingTreasuryForexDealsGridTbl_Id_"+_pageRef).unsubscribe("onPendingTreasuryForexDealsGridLoad");
	   var isNavConstructed = false;
		$("#pendingTreasuryForexDealsGridTbl_Id_"+_pageRef).subscribe("onPendingTreasuryForexDealsGridLoad",function(response,html){
			 if(!isNavConstructed)
			 {
				 $("#pendingTreasuryForexDealsGridTbl_Id_"+_pageRef).jqGrid ('navButtonAdd', '#pendingTreasuryForexDealsGridTbl_Id_'+_pageRef+'_pager',
											      {  
						 						  caption:"<ps:text name='Print_Statement__key' />",
												  buttonicon: "ui-icon-print",
												  title: "<ps:text name='Print_Statement__key' />",
											      onClickButton: function() {										      
											      printGrid("pendingTreasuryForexDealsGridTbl_Id_"+_pageRef,"<ps:text name='Pending_Deals_key' />",["action"]);
