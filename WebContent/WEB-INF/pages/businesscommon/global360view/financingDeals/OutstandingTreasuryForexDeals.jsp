...
$(document).ready(function() {	
	$("#outstandingTreasuryForexDealsSection_"+_pageRef+".collapsibleContainer").collapsiblePanel();

});

function treasuryForexDealsOutstandingGridActionFormatter(cellValue, options, rowObject){
	var hyperLinks="";
 ...
 <div id="outstandingTreasuryForexDealsSection_${_pageRef}" class="collapsibleContainer" title="<ps:text name="outstanding_deals_key" />" >
    
    <ps:url id="urlOutstandingTreasuryForexDealsGrid" action="financingDealsListAction_loadTreasuryForexDeals" namespace="/path/globalview" escapeAmp="false">
		 ...
		 <ps:param name="financingDealsSC.outstandingDealsFlag" value="'Y'"></ps:param>
	 </ps:url>
	 
	 <psjg:grid
	    	id				="outstandingTreasuryForexDealsGridTbl_Id_${_pageRef}" 
	  	    href            ="%{urlOutstandingTreasuryForexDealsGrid}"  	    
	        dataType        ="json"       
	    	pager           ="false"
	    	sortable        ="true"
			filter          ="false"
	        ...
	        shrinkToFit     ="false" 
			height="230"
			onCompleteTopics="onOutstandingTreasuryForexDealsGridLoad" >
	        ...
	        <psjg:gridColumn name="action"	title="%{getText('actions_key')}" index="action"  colType="custom" 	editable="false" 	sortable="false" 	search="false"	id="actionEdit" 	 formatter="treasuryForexDealsOutstandingGridActionFormatter"  width="100" />
	  </psjg:grid>
 </div>    
 ...
 $(document).ready(function () {
	   $("#outstandingTreasuryForexDealsGridTbl_Id_"+_pageRef).unsubscribe("onOutstandingTreasuryForexDealsGridLoad");
	   var isNavConstructed = false;
		$("#outstandingTreasuryForexDealsGridTbl_Id_"+_pageRef).subscribe("onOutstandingTreasuryForexDealsGridLoad",function(response,html){
			 if(!isNavConstructed)
			 {
				 $("#outstandingTreasuryForexDealsGridTbl_Id_"+_pageRef).jqGrid ('navButtonAdd', '#outstandingTreasuryForexDealsGridTbl_Id_'+_pageRef+'_pager',
											      {  
						 						  caption:"<ps:text name='Print_Statement__key' />",
												  buttonicon: "ui-icon-print",
												  title: "<ps:text name='Print_Statement__key' />",
											      onClickButton: function() {										      
											      printGrid("outstandingTreasuryForexDealsGridTbl_Id_"+_pageRef,"<ps:text name='Approved_Deals_key' />",["action"]);
